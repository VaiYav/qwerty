...
export default {
  ...
  methods: {
    ...
    updateData() {
      ...
      let tooltipContent = ''
      const tooltip = this.$refs.sparklineTooltip
      for (const datum in this.datum) {
        curData = null
        if (Object.prototype.hasOwnProperty.call(this.datum, datum)) {
          this.setStatus(false)
          for (const [index, pos] of this.datum[datum].points.entries()) {
      ...
      if (rect) {
        ...
        tooltip.style.top = `${this.curEvt.cy + window.scrollY - rect.height}px`
        try {

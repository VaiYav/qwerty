export const SET_CONFIG = 'SET_CONFIG'
export const SET_DATA = 'SET_DATA'
export const EDIT_ENTITY = 'EDIT_ENTITY'
export const SET_LOADER = 'SET_LOADER'
export const SELECT_ROW = 'SELECT_ROW'
export const REQUEST_SUCCESS = 'REQUEST_SUCCESS'
export const REQUEST_FAILURE = 'REQUEST_FAILURE'
export const TOGGLE_DEVICE = 'TOGGLE_DEVICE'
export const TOGGLE_CHECK_ALL = 'TOGGLE_CHECK_ALL'
export const SET_UN_SELECTED_ROW = 'SET_UN_SELECTED_ROW'
export const CHANGE_SORTING = 'CHANGE_SORTING'
export const SET_COLUMNS = 'SET_COLUMNS'
export const CHOOSE_FILTER = 'CHOOSE_FILTER'
export const SET_FILTERS = 'SET_FILTERS'
export const SET_DEFAULT_FILTERS = 'SET_DEFAULT_FILTERS'
export const SAVE_COLUMNS = 'SAVE_COLUMNS'
export const TOGGLE_EDIT_MODE = 'TOGGLE_EDIT_MODE'
export const CREATE_FILTER = 'CREATE_FILTER'
export const SEARCH_BY_FILTER = 'SEARCH_BY_FILTER'
export const SET_PAGINATION = 'SET_PAGINATION'
export const TOGGLE_FIXED_HEADER_STATUS = 'TOGGLE_FIXED_HEADER_STATUS'
export const CLONE_SAVED_FILTERS = 'CLONE_SAVED_FILTERS'
export const SET_STANDARD_FILTER = 'SET_STANDARD_FILTER'
